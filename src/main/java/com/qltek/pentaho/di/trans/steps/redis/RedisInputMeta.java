 ...
 */
@Step(id = "RedisInput",
        image = "redis-input.png",
        
        name = "Redis Input",
        description = "Reads from a Redis instance",
...
public class RedisInputMeta extends BaseStepMeta implements StepMetaInterface {
    ...
    private Set<CustomeHostAndPort> servers;

    public CustomeHostAndPort getJedisServer() {
        return jedisServer;
    }

    public void setJedisServer(CustomeHostAndPort jedisServer) {
        this.jedisServer = jedisServer;
    }

    private CustomeHostAndPort jedisServer;
    ...
    public void getFields(RowMetaInterface inputRowMeta, String origin, RowMetaInterface[] info, StepMeta nextStep,
        ...
        if (!Utils.isEmpty(this.valueFieldName)) {
    ...
    public String getXML() throws KettleException {
        ...
        Set<CustomeHostAndPort> servers = this.getServers();
        if (servers != null) {
            for (HostAndPort addr : servers) {
                ...
                retval.append("        ").append(XMLHandler.addTagValue("password", addr.getPort()));
                retval.append("      </server>").append(Const.CR);
            }
        }
        retval.append("    </servers>").append(Const.CR);
    ...
    private void readData(Node stepnode) throws KettleXMLException {
        try {
            ...
            for (int i = 0; i < nrservers; i++) {
                ...
                String password = XMLHandler.getTagValue(fnode, "password");
                CustomeHostAndPort hostAndPort = new CustomeHostAndPort(hostname, port, password);
                if (i == 0) {
    ...
    public void readRep(Repository rep, ObjectId id_step, List<DatabaseMeta> databases, Map<String, Counter> counters)
        ...
        try {
            ...
            allocate(nrservers);
            for (int i = 0; i < nrservers; i++) {
                servers.add(new CustomeHostAndPort(rep.getStepAttributeString(id_step, i, "hostname"), Integer.parseInt(rep
                        .getStepAttributeString(id_step, i, "port")), rep.getStepAttributeString(id_step, i, "password")));
            }
        ...
        try {
            ...
            if (servers != null) {
                for (HostAndPort addr : servers) {
                    ...
                    rep.saveStepAttribute(id_transformation, id_step, i++, "password", addr.getPort());
                }
            }
        } catch (Exception e) {
            throw new KettleException(BaseMessages.getString(PKG,
    ...
    public Set<CustomeHostAndPort> getServers() {
        return servers;
    }

    public void setServers(Set<CustomeHostAndPort> servers) {
